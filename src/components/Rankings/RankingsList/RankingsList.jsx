  ...
  return (
    <Wrapper>
      <InfoBar>
        ...
        <RightWrapper>
          {!isMobile && (
            <InfoBarPart w={isDesktop ? '160px' : isTablet ? '100px' : '80px'}>Change</InfoBarPart>
          )}
          {isDesktop && (
            <InfoBarPart w={isDesktop ? '160px' : isTablet ? '100px' : '80px'}>
              NFTs Created
            </InfoBarPart>
          )}
      ...
      <List>
        {rankings ? (
          rankings.map((artist, index) => (
            <RankingsListItem key={artist._id} artist={artist} index={index} />
          ))
        ) : (
