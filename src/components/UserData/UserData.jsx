    ...
    <>
      <Container>
        <CoverWrapper background={profileCover} show={show}>
          {edit && (
            <FileInputLabel>
              <EditIcon />
              <FileInput name="profileCover" type="file" onChange={handleImageEdit} />
            </FileInputLabel>
          )}
          <EditorBtn
            onClick={toggleEdit}
            text={edit ? 'Close editor' : 'Edit profile'}
          ...
          <InfoWrapper show={show}>
            <AvatarWrapper show={show}>
              ...
              {edit && (
                <FileInputLabel>
                  <EditIcon />
                  <FileInput name="avatarUrl" type="file" onChange={handleImageEdit} />
                </FileInputLabel>
              )}
            </AvatarWrapper>
            <UsernameBtnsWrapper>
              ...
              <EditWrapper>
                ...
                {edit && !edField && (
                  <IconButton
                    ...
                    position="static"
                    ml="10px"
                    onClick={() => toggleEditField('socialLinks')}
                  />
                )}
              </EditWrapper>
              <UserSocialList
                socialLinks={socialLinks}
                editing={edField === 'socialLinks'}
                addLink={addLink}
      ...
      {addOpen &&
        createPortal(
          <NftModal
            type="add"
            initialState={addModalIinitialState}
            modalOpen={addOpen}
            toggleModal={toggleAddOpen}
            show={modalShow}
            onSubmit={handleNftAdd}
            sumbitText="Add NFT"
          />,
      ...
      {collectionOpen &&
        createPortal(
          <CollectionModal
            ...
            toggleModal={toggleCollectionOpen}
            show={modalShow}
          />,
          document.querySelector('#modal-root')
        )}
